...
-2.4288314855, -2.2200919553, -1.8547257521, -1.7473247141, -1.7465285986, -1.7406443388, -1.7235993891, -1.6854530173, -1.6028581134, -1.4917048930, -1.4767443251, -1.4376808383, -1.3521549811, -1.3487522215, -1.3372159217, -1.0955078920, -1.0784611231, -1.0423773226, -1.0016687228, -0.9766797163, -0.8866419096, -0.8005035932, -0.7408119479, -0.7164440467, -0.7150722546, -0.6700113252, -0.6263447809, -0.6011187943, -0.5899019224, -0.5783118283, -0.5694084336, -0.5399571922, -0.4901075119, -0.4456101964, -0.4266358195, -0.4212868011, -0.3733556815, -0.3172085898, -0.2588617204, -0.2139585286, -0.1941537120, -0.1866998222, -0.1185661060, -0.0990408737, -0.0916497560, -0.0607272265, -0.0463795206, -0.0443787892, -0.0276632985, -0.0054876268,  0.0006166116,  0.0056597222,  0.0131847490,  0.0403523690,  0.0522796284,  0.0745620755,  0.0804426849,  0.0822682864,  0.0892373508,  0.1540986151,  0.2043294988,  0.2071935391,  0.2085606838,  0.2316877827,  0.2996455612,  0.3047844542,  0.3272821663,  0.3511956136,  0.3721299719,  0.4091298106,  0.4210526396,  0.4783000451,  0.5412750386,  0.5622796280,  0.5927251657,  0.6645578034,  0.6693057720,  0.6791562539,  0.7820694005,  0.8015172524,  0.8392994135,  0.8526843570,  0.9476941142,  0.9778736730,  1.0108399089,  1.1700898454,  1.1834549656,  1.1877803545,  1.3179208176,  1.3697384613,  1.4255614385,  1.5799665186,  1.6269383089,  1.7725670430,  1.9113730397,  2.0787986791,  2.2991867011,  2.4874321107,  2.9620978930,  3.5846890041
};
...
TEST(Entropy, Null)
{
	double e = Entropy::entropy(a,2);
	EXPECT_NEAR(e, 0.0, TOLERANCE);
}

TEST(Entropy, Easy)
{
	double e = Entropy::entropy(b,2);
	EXPECT_NEAR(e, 0.6730117, TOLERANCE);
}

TEST(Entropy, EasyNegative)
{
	double e = Entropy::entropy(c,2);
	EXPECT_NEAR(e, 0.6730117, TOLERANCE);
}

TEST(Entropy, Random100)
{
	double en = Entropy::entropy(e, 20);
	EXPECT_NEAR(en, 0.3339327, TOLERANCE);
}
...
TEST(ConditionalCummulativeEntropyEstimatorKruskall, Null)
{
	...
	v.emplace_back(a);
	v.emplace_back(a);
	std::vector<double> ve = Entropy::conditional_cummulative_entropy_kruskall_estimator(a,v);
	for(auto e : ve){
		EXPECT_NEAR(e, 0.0, TOLERANCE);
	}
}
...
TEST(ConditionalCummulativeEntropyEstimatorKruskall, SameVector)
{
	std::vector<std::vector<double>> v;
	v.emplace_back(b);
	std::vector<double> ve = Entropy::conditional_cummulative_entropy_kruskall_estimator(b,v);
	...
	v.emplace_back(b);
	v.emplace_back(b);
	ve = Entropy::conditional_cummulative_entropy_kruskall_estimator(b,v);
	EXPECT_NEAR(ve[0], 0.0, TOLERANCE);
	EXPECT_NEAR(ve[1], 0.138629, TOLERANCE);
	EXPECT_NEAR(ve[2], 0.381909, TOLERANCE);
	EXPECT_NEAR(ve[3], 0.727127, TOLERANCE);
...
TEST(ConditionalCummulativeEntropyEstimatorKruskall, ConditionedOnTwoVectors10)
{
	...
	v.emplace_back(y);
	v.emplace_back(z);
	std::vector<double> ve = Entropy::conditional_cummulative_entropy_kruskall_estimator(x,v);
