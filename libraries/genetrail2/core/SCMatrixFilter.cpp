...
namespace GeneTrail
{
	 void SCMatrixFilter::filterMatrix(DenseMatrix& matrix, const FilterParams& params){
		...
		fillColumnStatistics(matrix, total_count, mito_count, nonzero_features, params);
		
		std::cout << "Filtering cells..." << std::endl;
		std::vector<std::string> keep;
		std::vector<size_t> keep_idx;
		for(size_t idx_cell=0; idx_cell < matrix.cols(); idx_cell++){
			if(passFilter(total_count[idx_cell], nonzero_features[idx_cell], params)){
	...
	void SCMatrixFilter::fillColumnStatistics(const DenseMatrix& matrix, std::vector<double>& total_count, std::vector<double>& mito_count,
	...
	bool SCMatrixFilter::passFilter(double total_count, double nonzero_features, const FilterParams& params){
