...
class GT2_EXPORT GeneSet
{
  ...
  public:
	 ...
	 *
	 * @param decreasing Boolean flag indication how to sort the scores (true =
	 *decreasing).
	 ...
	 * @return Sorted vector of identifier/score pairs.
	 */
	Container sortAndIntersect(const std::set<std::string>& set,
	 ...
	 * @return A reference to the gene set. This can be used for chaining.
	 */
	GeneSet& filter(GeneSetFilter::GeneSetFilter& gf) { return filter(&gf); }
 ...
 *          real value.
 */
template <typename Func> void transform(GeneSet& gene_set, Func f)
{
